...
/**
 * Server function exposed to the route loader / `useQuery`. Wraps the
 * internal `fetchAndCacheGitHubHealth` — keeping logic in a plain async
 * function lets us test it directly without standing up the Start
 * AsyncLocalStorage context.
 */
export const getGitHubHealth = createServerFn({ method: "GET" })
	.inputValidator(detailSchema.parse)
	.handler(async ({ data }) => fetchAndCacheGitHubHealth(data.repoUrl));
 ...
 *    Shared across users; survives server restarts and React Query cache
 *    wipes.
 * 2. React Query on the client (configured in the caller). 1h staleTime
 *    mirrors the DB cache.
 *
 * Anonymous GitHub calls get a 60 req/hr quota; setting `GITHUB_TOKEN` in
 * env raises that to 5000. Failed lookups (404, 403/rate-limit, network)
 * return `null` and are intentionally NOT cached — a transient failure
...
/**
 * Strip URL-noise so we get one cache row per repo regardless of how the
 * forge link was written (.git suffix, trailing slash, https vs http, etc.).
 */
...
async function fetchFromGitHub(
	...
	// Latest commit on default branch — best-effort, separate request so a
	// failure here doesn't sink the repo metadata.
